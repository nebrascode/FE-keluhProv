...
const Content = ({ complaint }) => {
  ...
  // Tampilkan loading jika complaint tidak ada
  if (!complaint) {
    return <p>Loading...</p>;
  }

  // Pastikan complaint.files adalah array
  const files = Array.isArray(complaint.files) ? complaint.files : [];
      ...
      <>
        <div className="lg:w-[500px] h-[500px] md:w-full bg-[#E6E0E9] p-2 rounded-lg drop-shadow-lg flex flex-col">
          <div className="flex-1 mb-2 overflow-hidden rounded-lg">
            <img
                ...
                src={files.length > 0 ? `https://storage.googleapis.com/e-complaint-assets/${files[currentImage].path}` : ""}
          ...
          <div className="relative w-full" data-carousel="static">
            ...
            <button
                ...
                data-carousel-prev
                onClick={() =>
                    handleChange(currentImage === 0 ? files.length - 1 : currentImage - 1)
                }
            >
            ...
            <button
                ...
                data-carousel-next
                onClick={() =>
                    handleChange((currentImage + 1) % files.length)
                }
            >
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
              <svg
        ...
        <div className="flex flex-col gap-2 w-full h-auto lg:w-[80%] lg:h-[500px]">
          <section className="bg-[#E6E0E9] py-1 px-4 rounded-t-lg">
            <h6>Pengadu</h6>
            {/* Ubah di sini jika tidak ada `user` */}
            <p>{complaint.user_id || "Nama tidak tersedia"}</p>
          </section>
          <section className="w-full">
            <div className="w-full flex flex-col lg:flex-row gap-2">
              ...
              <section className="bg-[#E6E0E9] py-1 px-4 rounded-t-lg w-full">
                <h6>Lokasi</h6>
                {/* Ubah di sini jika tidak ada `regency` */}
                <p>{complaint.regency_id || "Lokasi tidak tersedia"}</p>
              </section>
            </div>
          </section>
          <section className="bg-[#E6E0E9] py-1 px-4 rounded-t-lg w-full">
